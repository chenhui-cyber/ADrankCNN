...
import pandas as pd
from datasets.ADNI_single import AdniDataSet
...
if __name__ == "__main__":
    ...
    ADNI2_DATA = pd.read_csv("/data1/qiaohezhe/MRI_MMSE/time_data/ADNIMERGE_ADNI1_BL_PROCESSED.csv")
    ADNI1_DATA = pd.read_csv("/data1/qiaohezhe/MRI_MMSE/time_data/ADNIMERGE_ADNI2_BL_PROCESSED.csv")
    ...
    for epoch in range(epochs):
        ...
        for i, data in tqdm(enumerate(train_loader), total=len(train_loader)):
            inputs, labels, group = data
            inputs, labels, group = inputs.to(device), labels.to(device), group.to(
                device)
            optimizer.zero_grad()
            logps, merged, outline = model.forward(inputs)

            '''mse loss'''
            loss_mmse = F.mse_loss(logps, labels.float())

            '''cross loss'''
            loss_group = criterion(outline, group)
            ...
            loss_mmse = 0 * loss_line + 0 * loss_mmse + 1 * loss_pair
            loss_mmse.backward()
            optimizer.step()
            running_loss_mmse += loss_mmse.item()
            running_loss_group += loss_group
    ...
    #         # label_predict.to_csv("/data1/qiaohezhe/wise_dnn/log/corr/adni2_bl/correlation_{}.csv".format(epoch), mode='w',
    ...
    #
    #         result_list.append(
    #             [epoch, train_loss, pair_loss, tripet_loss, branch_loss, val_loss, val_branch, val_line, corr])
    #
    ...
    #
    #         result.to_csv("/data1/qiaohezhe/wise_dnn/log/fcn_compare_branch_sub_threshold_adni2_bl_8.csv", mode='w',
    #                       index=False, header=True)
    ...
    #         # if epoch > 30:
    #         # torch.save(model,
    #         # "/data1/qiaohezhe/wise_dnn/log/fcn_branch_compair_adni1_{}.pth".format(epoch))
    #
    # stop = datetime.now()
