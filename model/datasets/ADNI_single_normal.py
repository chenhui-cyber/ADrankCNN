...
class AdniDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_array2 = self.__training_data_process__(image_subject)
        patch_shape = (36, 36, 36)
        extraction_step = 20
        landmk_num = 3 * 3 * 3
        patches = np.zeros((landmk_num, 1, patch_shape[0], patch_shape[1], patch_shape[2]),
                           dtype='uint8')
        # img_patches = extract_patches(img_array2, patch_shape, extraction_step)
        ...
        # sig = 2
        # distribution = [i for i in range(35)]
        # for i in distribution:
        #     distribution[i] = np.exp(-(int(output) - distribution[i]) ** 2 / (2 * sig ** 2)) / (
        #                 math.sqrt(2 * math.pi) * sig)
        #     # distribution[i] = np.exp(-(int(output) - distribution[i]) ** 2 /(2 * sig ** 2))
        #
        # distribution = softmax(np.array(distribution), 0)

        k = 5

        z1 = int(output / k)
        z2 = int(output / k) + 1
        label1 = 1 - (output - int(output / k) * k) / k
        label2 = 1 - ((int(output / k) + 1) * k - output) / k

        distribution = np.zeros(8)
        distribution[z1] = label1
        distribution[z2] = label2

        branch = np.zeros(8)
        branch[:z1] = 1
        # branch[z2] = 1
        return img_array1, output, distribution, branch
