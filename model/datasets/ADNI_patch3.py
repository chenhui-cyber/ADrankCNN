...
class data_flow(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sub_extraction_step = 20
        sub_img_patches = extract_patches(image_subject, patch_shape, sub_extraction_step)
        sub_patch_shape = (12, 12, 12)
        sub_landmk_index = 0
        sub_landmk_num = 3 * 3 * 3
        sub_patches = np.zeros((sub_landmk_num, 1, sub_patch_shape[0], sub_patch_shape[1], sub_patch_shape[2]),
            dtype='uint8')
        for sub_i in range(sub_img_patches.shape[0]):
            for sub_j in range(sub_img_patches.shape[1]):
                for sub_k in range(sub_img_patches.shape[2]):
                    sub_patches[landmk_index, 0, ...] = sub_img_patches[sub_i, sub_j, sub_k, ...]
                    sub_landmk_index += 1

        return patches, sub_patches, output, group, np.expand_dims(image_subject, axis=0)
