...
import pandas as pd
from datasets.ADNI_single6 import AdniDataSet
...
if __name__ == "__main__":
    ...
    # IMAGE2_PATH = "/data1/qiaohezhe/MRI_MMSE/BL818_processed/"
    # IMAGE1_PATH = "/data1/qiaohezhe/MRI_MMSE/BL776_processed/"
    #
    # ADNI2_DATA = pd.read_csv("/data1/qiaohezhe/MRI_MMSE/time_data/ADNIMERGE_ADNI1_24_PROCESSED.csv")
    # ADNI1_DATA = pd.read_csv("/data1/qiaohezhe/MRI_MMSE/time_data/ADNIMERGE_ADNI2_24_PROCESSED.csv")

    ADNI2_DATA = ADNI2_DATA.dropna()
    ADNI1_DATA = ADNI1_DATA.dropna()

    TRN_LBLS = ADNI1_DATA['MMSE_06'].tolist()
    VAL_LBLS = ADNI2_DATA['MMSE_06'].tolist()
    ...
    for epoch in range(epochs):
        ...
        for i, data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            group_compare[torch.abs(labels1 - labels2) <= 3] = 0.5
            group_compare[labels1 - labels2 < -3] = 0
            group_compare[labels1 - labels2 > 3] = 1
            print(group_compare)
            print(logps_sig)
            ...
            print("loss_tripet1", loss_pair)

            loss_mmse = F.mse_loss(logps, labels.float())
            loss_line = F.mse_loss(outline, labels_line.float())
            ...
            loss_mmse = 0 * loss_line + 0 * loss_mmse + 1 * loss_pair + 1 * loss_branch
            loss_mmse.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            label_predict.to_csv("/data1/qiaohezhe/wise_dnn/log/corr/adni1_06/correlation_{}.csv".format(epoch),
                                 mode='w',
            ...
            result_list.append([epoch, train_loss, pair_loss, tripet_loss, branch_loss, val_loss, val_branch, val_line, corr])

            name = ['epoch', 'train_loss', 'pair_loss', 'tripet_loss', 'loss_branch', 'val_loss', 'val_branch', 'val_line', 'correlation']
            result = pd.DataFrame(columns=name, data=result_list)
            early_stopping(val_loss, model)

            result.to_csv("/data1/qiaohezhe/wise_dnn/log/fcn_compair_branch_corr_adni1_06.csv", mode='w',
                          index=False, header=True)
            ...
            # if epoch > 30:
            #     torch.save(model, "/data1/qiaohezhe/wise_dnn/log/fcn_branch_adni1_12_{}.pth".format(epoch))
